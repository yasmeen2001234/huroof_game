        ...
        child: SingleChildScrollView(
          ...
          child: Column(children: [
              ...
              children: List.generate(
                  ...
                  (i) => Padding(
                        ...
                        child: Transform.rotate(
                            angle: (i - 1) * 0.12, child: const FaceDownCard()),
                      )
                          .animate()
            ...
            TextButton.icon(
              ...
              label: const Text('🛠 Debug: view all rooms',
...
class _Btn extends StatelessWidget {
  ...
  Widget build(BuildContext context) => SizedBox(
        ...
        child: ElevatedButton(
          ...
          child: Text(label,
              style: arabicStyle(fontSize: 16, weight: FontWeight.w900),
              textDirection: TextDirection.rtl),
        ),
      );
}
...
class _GameRouterState extends ConsumerState<GameRouter> {
  List<StreamSubscription> _presenceSubs = [];
  ...
  void initState() {
    super.initState();
    final service = ref.read(gameServiceProvider);
    _presenceSubs = service.watchPresence(widget.gameId, _onRoomEmpty);
    _gameDeletedSub = service.watchGameDeleted(widget.gameId, _onRoomEmpty);
  }
  ...
  Widget build(BuildContext context) {
    ...
    return PopScope(
      ...
      child: gameAsync.when(
        loading: () =>
            _bg(const CircularProgressIndicator(color: Colors.white)),
        error: (e, _) =>
            _bg(Text('خطأ: $e', style: const TextStyle(color: Colors.white))),
        data: (game) {
          // Show lobby when waiting or no round started yet
          if (game.currentState == RoundState.waiting ||
              game.currentRound == 0) {
            return LobbyScreen(
                gameId: widget.gameId, onLeave: _leaveExplicitly);
          }

          return roundAsync.when(
            // While loading the new round doc, show a spinner NOT lobby
            // This prevents the brief "null round → lobby" flicker
            loading: () =>
                _bg(const CircularProgressIndicator(color: Colors.white)),
            error: (e, _) => _bg(
                Text('خطأ: $e', style: const TextStyle(color: Colors.white))),
            data: (round) {
              // Round doc not created yet — keep showing spinner
              if (round == null) {
                return _bg(
                    const CircularProgressIndicator(color: Colors.white));
              }

              // Use round.state as source of truth (not game.currentState)
              // This avoids race condition between the two streams
              switch (round.state) {
